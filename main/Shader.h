...
public:
	...
	virtual bool scatter(const Ray& inRay, const HitData& hitdata, std::vector<vector3>& outAbsorbtion, vector<Ray>& outRays) const = 0;
...
public:
	...
	virtual bool scatter(const Ray& inRay, const HitData& hitdata, vector<vector3>& outAbsorbtion, vector<Ray>& outRays) const {
...
inline float fresnel(const vector3& in, const vector3& surfaceNormal, float ior) {
	...
	float sinOut = iorIn / iorOut * sqrtf(max(0.f, 1 - cosIn * cosIn)) ;
	...
	if (sinOut >= 1) {
		...
		float cosOut = sqrtf(max(0.f, 1 - sinOut * sinOut));
		cosIn = fabsf(cosIn);
