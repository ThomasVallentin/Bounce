...
void printTimeInfo(const std::chrono::time_point<std::chrono::high_resolution_clock>& startTime, unsigned short percentage)
{
...
bool RayTracer::trace(const Camera& camera)
{
	...
	// Progress bar
	unsigned short progress(0), displayed_progress(0);
    ...
    for (int s = 0; s < m_samples; s++)
    {
        for (int y = 0; y < m_height; y++)
        {
            for (int x = 0; x < m_width; x++)
            {
                ...
                progress = int((s / float(m_samples)) * 100);
                if (progress > displayed_progress) {
                    std::cout << "Rendering " << displayed_progress << "%... ";
                    displayed_progress += 5;
